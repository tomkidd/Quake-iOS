...
void Host_Savegame_f (void)
{
	...
	sprintf (name, "%s/%s_%s", sys_documentsdir, selected_game_dir, Cmd_Argv(1));
	COM_DefaultExtension (name, ".sav");
	
	Con_Printf ("Saving game to %s_%s.sav...\n", selected_game_dir, Cmd_Argv(1));
	f = fopen (name, "w");
	if (!f)
	{
...
//	SCR_BeginLoadingPlaque ();
	...
	Con_Printf ("Loading game from %s_%s.sav...\n", selected_game_dir, Cmd_Argv(1));
	f = fopen (name, "r");
	if (!f)
	{
		Con_Printf ("ERROR: couldn't open.\n");
...
void Host_Name_f (void)
{
	...
	host_client->edict->v.netname = (int)(host_client->name - pr_strings);
...
// run the entrance script
	...
	else
	{
		...
		ent->v.netname = (int)(host_client->name - pr_strings);
...
// with a permanent head tilt
	ent = EDICT_NUM( (int)(1 + (host_client - svs.clients)) );
