...
void BGM_Play (const char *filename)
{
    ...
    {
        if (handler->is_available &&
            !Q_strcasecmp((char *)ext, (char *)handler->ext))
        break;
        handler = handler->next;
    }
    if (!handler)
...
//        q_snprintf(tmp, sizeof(tmp), "%s/id1/%s/track%02d.%s",
//                   host_parms.basedir, MUSIC_DIRNAME, (int)track, handler->ext);
        Con_Printf("Trying to play file %s\n", tmp);
